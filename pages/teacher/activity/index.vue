...
async function fetchStudentWithRid() {
    try {
        const res = await $axios.get("/student/with_rid");
        const data = res.data;
        // console.log(data);

        Object.assign(studentDataWithRid.value, data);
        // console.log(studentDataWithRid.value);
    } catch (err) {
        console.log(err);
    }
}
...
//   r.push(`/teacher/activity/${params}`);
// }
...
async function createActivitySubmit() {
    try {
        ...
        const res = await $axios.post("/teacher/activity", formData, { headers: { "Content-Type": "multipart/form-data" } });
        if (res.status === 200) {
...
async function editActivitySubmit(aid) {
    try {
        ...
        const res = await $axios.put(`/teacher/activity/${aid}`, formData, { headers: { "Content-Type": "multipart/form-data" } });
        if (res.status === 200) {
    ...
    <div class="min-h-full bg-gradient-to-b from-blue-100 to-white rounded">
        ...
        <dialog v-if="viewActivityModalShow" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 w-full h-full" :class="{ active: viewActivityModalShow }" open>
            <div class="bg-white rounded-lg shadow-lg w-full max-w-screen-xl h-[90vh] mx-2 sm:mx-6 overflow-y-auto">
                <div class="p-4 sm:p-6">
                    <div class="flex justify-between items-center mb-4">
                        <h3 class="text-lg sm:text-xl font-semibold text-gray-800">{{ viewActivityData.activity_name }}</h3>
                        <button
                    ...
                    <div class="overflow-x-auto">
                        <div class="bg-white rounded-lg shadow-lg mx-2 sm:mx-6 overflow-y-auto p-4 sm:p-6">
                            <div>
                                ...
                                <div class="p-4">
                                    <p class="text-sm sm-text-xs font-regular text-grey-600">{{ viewActivityData.activity_description }}</p>
                                </div>
                            </div>
                            <div>
                                <h5 class="text-md sm:text-lg font-semibold text-gray-800">สถานที่</h5>
                                <div class="p-4">
                                    <p class="text-sm sm-text-xs font-regular text-grey-600">{{ viewActivityData.activity_location || "ไม่ระบุ" }}</p>
                                </div>
                            </div>
                            <div>
                                <h5 class="text-md sm:text-lg font-semibold text-gray-800">ผู้จัด</h5>
                                <div class="p-4">
                                    <p class="text-sm sm-text-xs font-regular text-grey-600">{{ viewActivityData.activity_organization || "ไม่ระบุ" }}</p>
                                </div>
                            </div>
                            <div>
                        ...
                        <div class="flex justify-between my-4">
                            <p class="text-xs text-green-600">วันเริ่มต้นกิจกรรม : {{ viewActivityData.activity_start_date || "ไม่ระบุ" }}</p>
                            <p class="text-xs text-yellow-600">วันสิ้นสุดกิจกรรม : {{ viewActivityData.activity_end_date || "ไม่ระบุ" }}</p>
                        </div>
                        ...
                        <table class="min-w-full bg-white rounded-lg overflow-hidden text-sm sm:text-base">
                            ...
                            <tbody class="divide-y divide-gray-200">
                                <tr v-for="(item, index) in studentDataWithRid" :key="index + 1" class="hover:bg-gray-50 dtransition-all uration-200">
                                    <td class="py-2 sm:py-3 px-2 sm:px-4 truncate transition-transform duration-200 hover:scale-105">
