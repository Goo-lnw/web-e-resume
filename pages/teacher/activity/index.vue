...
async function submitSelectedStudents() {
  const selectedStudents = studentDataWithRid.value.filter(
  ...
  if (res.status === 200) {
    showAlert("มอบเกียรติบัตรเรียบร้อย", "success");
    assignCertModalShow.value = false;
    viewActivityModalShow.value = false;
    await fetchStudentWithRid();
    await fecthActivityById(id_activity.value);
  } else {
...
async function fetchStudentWithRid() {
  try {
    ...
    studentDataWithRid.value = data.map((student) => ({
      ...student,
      selected: false,
    }));
...
<template>
  <div class="min-h-full bg-gradient-to-b from-blue-100 to-white rounded">
    ...
    >
      <div
        class="bg-white rounded-lg shadow-lg w-full max-w-screen-xl h-[90vh] mx-2 sm:mx-6 overflow-y-auto"
      >
        <div class="p-4 sm:p-6">
          <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg sm:text-xl font-semibold text-gray-800">
          ...
          <div class="overflow-x-auto">
            <div
              class="bg-white rounded-lg shadow-lg mx-2 sm:mx-6 overflow-y-auto p-4 sm:p-6"
            >
              <div>
                <h5 class="text-md sm:text-lg font-semibold text-gray-800">
                  รายละเอียด
                </h5>
                <div class="p-4">
            ...
            >
              ...
              <tbody class="divide-y divide-gray-200">
                <tr
                  v-for="(item, index) in studentDataWithRid"
                  :key="index + 1"
                  class="hover:bg-gray-50 dtransition-all uration-200"
                >
                  <td
