...
function viewCertificate(training) {
  const name = btoa(encodeURIComponent(resume.value.student?.student_name || "Unknown"));
  const file = btoa(encodeURIComponent(training.training_history_certificate_file));
    ...
    <div v-else class="max-w-7xl mx-auto p-6 space-y-8">
      ...
      <div class="bg-white shadow-lg rounded-lg p-6">
        <div class="flex flex-col md:flex-row items-center md:items-start gap-6">
          <div class="w-24 h-24 rounded-full overflow-hidden bg-gradient-to-r from-blue-500 to-indigo-500 flex items-center justify-center">
            <NuxtImg v-if="resume.student?.student_profile_image" :src="resume.student.student_profile_image" :alt="`${resume.student?.student_name} Profile`" class="w-full h-full object-cover" />
          ...
          <div class="flex-1 text-center md:text-left">
            ...
            <!-- Resume Status -->
            <div class="mb-4">
              <span class="px-3 py-1 rounded-full text-sm font-medium" :class="getStatusBadge(resume.resume?.resume_status)">
                {{ resume.resume?.resume_status?.toUpperCase() || $t("resume_detail.unknown") }}
              </span>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-1 space-y-6">
          <!-- Contact Information -->
          <div class="bg-white shadow-lg rounded-lg p-6">
            <h2 class="text-xl font-semibold mb-4 text-blue-600 flex items-center gap-2">📧 {{ $t("resume_detail.contact_info") }}</h2>
            <div class="space-y-3">
          ...
          <!-- Skills -->
          <div class="bg-white shadow-lg rounded-lg p-6">
            <h2 class="text-xl font-semibold mb-4 text-blue-600 flex items-center gap-2">💻 {{ $t("resume_detail.skills") }}</h2>
            <div class="space-y-4">
              <div v-for="(skill, index) in resume.skills?.filter((s) => s.skill_name)" :key="index" class="space-y-2">
          ...
          <!-- Soft Skills -->
          <div class="bg-white shadow-lg rounded-lg p-6">
            <h2 class="text-xl font-semibold mb-4 text-purple-600 flex items-center gap-2">🧠 {{ $t("resume_detail.soft_skills") }}</h2>
            <div class="space-y-3">
          ...
          <div class="bg-white shadow-lg rounded-lg p-6">
            <h2 class="text-xl font-semibold mb-4 text-indigo-600 flex items-center gap-2">📎 {{ $t("resume_detail.additional_info") }}</h2>
            <div class="space-y-3">
              <div v-for="(info, index) in resume.additionalInfo?.filter((i) => i.additional_info_title)" :key="index">
                ...
                <a v-if="info.additional_info_file_attachment" :href="info.additional_info_file_attachment" target="_blank" class="text-blue-500 hover:text-blue-700 text-sm underline">
        ...
        <div class="lg:col-span-2 space-y-6">
          <!-- Work Experience -->
          <div class="bg-white shadow-lg rounded-lg p-6">
            <h2 class="text-xl font-semibold mb-4 text-orange-600 flex items-center gap-2">💼 {{ $t("resume_detail.work_experience") }}</h2>
            <div class="space-y-4">
              <div v-for="(work, index) in resume.workExperience?.filter((w) => w.work_experience_company_name)" :key="index" class="border-l-4 border-orange-400 pl-4 py-2">
                ...
                <p v-if="work.work_experience_highlight" class="text-sm text-green-600 font-medium">🏆 {{ work.work_experience_highlight }}</p>
              </div>
          ...
          <!-- Projects -->
          <div class="bg-white shadow-lg rounded-lg p-6">
            <h2 class="text-xl font-semibold mb-4 text-green-600 flex items-center gap-2">🚀 {{ $t("resume_detail.projects") }}</h2>
            <div class="grid gap-4">
              <div v-for="(project, index) in resume.projects?.filter((p) => p.project_name)" :key="index" class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-2">
                  <h3 class="font-semibold text-gray-800">
                    {{ project.project_name }}
                  </h3>
                  <a v-if="project.project_attachment_link" :href="project.project_attachment_link" target="_blank" class="text-blue-500 hover:text-blue-700 text-sm underline">
                ...
                <p v-if="project.project_impact" class="text-sm text-green-600">{{ $t("resume_detail.impact") }}: {{ project.project_impact }}</p>
              </div>
          ...
          <!-- Education -->
          <div class="bg-white shadow-lg rounded-lg p-6">
            <h2 class="text-xl font-semibold mb-4 text-indigo-600 flex items-center gap-2">🎓 {{ $t("resume_detail.education") }}</h2>
            <div class="space-y-4">
              <div v-for="(edu, index) in resume.education?.filter((e) => e.education_history_institution)" :key="index" class="border-l-4 border-indigo-400 pl-4 py-2">
          ...
          <!-- Training -->
          <div class="bg-white shadow-lg rounded-lg p-6">
            <h2 class="text-xl font-semibold mb-4 text-purple-600 flex items-center gap-2">📚 {{ $t("resume_detail.training") }}</h2>
            <div class="space-y-4">
              <div v-for="(training, index) in resume.training?.filter((t) => t.training_history_course_name)" :key="index" class="border border-gray-200 rounded-lg p-4">
                ...
                <button v-if="training.training_history_certificate_file" @click="viewCertificate(training)" class="text-blue-500 hover:text-blue-700 text-sm underline" style="background: none; border: none; padding: 0; cursor: pointer">
          ...
          <!-- Internships -->
          <div class="bg-white shadow-lg rounded-lg p-6">
            <h2 class="text-xl font-semibold mb-4 text-teal-600 flex items-center gap-2">🏢 {{ $t("resume_detail.internships") }}</h2>
            <div class="space-y-4">
              <div v-for="(internship, index) in resume.internships?.filter((i) => i.internship_company_name)" :key="index" class="border-l-4 border-teal-400 pl-4 py-2">
                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-2">
                  <div>
                    <h3 class="font-semibold text-gray-800">
                ...
                <a v-if="internship.internship_related_files" :href="internship.internship_related_files" target="_blank" class="text-blue-500 hover:text-blue-700 text-sm underline">
      ...
      <div class="bg-white shadow-lg rounded-lg p-6">
        <h2 class="text-xl font-semibold mb-4 text-red-600 flex items-center gap-2">📝 {{ $t("resume_detail.teacher_evaluation") }}</h2>
          ...
          <div class="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
            <div class="flex gap-6">
              <label class="flex items-center gap-2 cursor-pointer">
                <input v-model="formComment.resume_status" type="radio" value="3" class="w-4 h-4 text-green-600 focus:ring-green-500" />
              ...
              <label class="flex items-center gap-2 cursor-pointer">
                <input v-model="formComment.resume_status" type="radio" value="4" class="w-4 h-4 text-red-600 focus:ring-red-500" />
            ...
            <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-lg transition-colors duration-200 font-medium">
