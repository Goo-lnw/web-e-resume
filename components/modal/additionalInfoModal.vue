...
function syncRefData(target, source) {
  ...
  const filteredTarget = target.filter((targetItem) => {
    ...
    if (!exists) {
      console.log(
        "Removing additional_info_id:",
        targetItem.additional_info_id
      );
    }
    return exists;
  });
  ...
  const remainingTargetIds = new Set(
    filteredTarget.map((item) => item.additional_info_id)
  );
  ...
  for (const sourceItem of source) {
    if (!remainingTargetIds.has(sourceItem.additional_info_id)) {
      console.log(
        "Adding new additional_info_id:",
        sourceItem.additional_info_id
      );
