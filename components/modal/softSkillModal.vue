        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-4xl w-full overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-purple-600 to-pink-600 px-2 py-2 md:px-6 md:py-4 text-white">
                <div class="flex items-center justify-between">
                    <div>
                        <h2 class="text-md md:text-2xl font-bold">{{ $t("soft_skill_modal.title") }}</h2>
                        <p class="text-purple-100 text-sm mt-1">{{ $t("soft_skill_modal.subtitle") }}</p>
                    </div>
            ...
            <div class="p-2 md:p-4 overflow-y-auto space-y-2 max-h-[calc(90vh-120px)]">
                <div class="space-y-2">
                    ...
                    <div
                        ...
                        class="group bg-gradient-to-r from-gray-50 to-gray-100 hover:from-purple-50 hover:to-pink-50 border border-gray-200 hover:border-purple-300 rounded-xl p-2 md:p-4 transition-all duration-200 hover:shadow-md"
                    >
                        <!-- Soft Skill Header -->
                            ...
                            <div class="flex items-center space-x-3 min-w-0">
                                ...
                                <div class="flex-1 min-w-0">
                                    <h3 class="font-semibold text-gray-800 break-words">
                                        {{ softSkill.soft_skill_name || $t("soft_skill_modal.soft_skill_no", { n: index + 1 }) }}
                                    </h3>
                                    <p class="text-xs text-nowrap text-gray-500">{{ $t("soft_skill_modal.skill_detail") }}</p>
                                </div>
                            </div>

                            <div class="flex items-center space-x-1 md:space-x-2 mb-3">
                                <span class="px-2 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800 text-nowrap">
                                    {{ $t("soft_skill_modal.soft_skill_label") }}
                                </span>
                                <button
                                    ...
                                    :title="$t('soft_skill_modal.remove_soft_skill')"
                                >
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div class="space-y-2">
                                <label class="text-sm font-medium text-gray-700 flex items-center space-x-2">
                                    ...
                                    <span>{{ $t("soft_skill_modal.soft_skill_name") }}</span>
                                </label>
                                <input
                                    v-model="softSkill.soft_skill_name"
                                    type="text"
                                    :placeholder="$t('soft_skill_modal.soft_skill_name_placeholder')"
                                    max="50"
                            ...
                            <div class="space-y-2">
                                <label class="text-sm font-medium text-gray-700 flex items-center space-x-2">
                                    ...
                                    <span>{{ $t("soft_skill_modal.soft_skill_description") }}</span>
                                </label>
                                <input
                                    v-model="softSkill.soft_skill_description"
                                    type="text"
                                    :placeholder="$t('soft_skill_modal.soft_skill_description_placeholder')"
                                    class="w-full text-xs px-2 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 bg-white hover:border-gray-400"
                                />
                            </div>
                        </div>
                        ...
                        <div class="mt-4">
                            <div class="flex items-center space-x-3">
                                <span class="text-xs text-gray-600">{{ $t("soft_skill_modal.status") }}</span>
                                ...
                                <span class="text-xs text-gray-500">
                                    {{ softSkill.soft_skill_name && softSkill.soft_skill_description ? $t("soft_skill_modal.complete") : softSkill.soft_skill_name ? $t("soft_skill_modal.partial") : $t("soft_skill_modal.incomplete") }}
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    >
                        <div class="flex flex-col items-center justify-center text-center">
                            ...
                            <h3 class="text-lg font-semibold text-purple-800 mb-2">
                                {{ isSaving ? $t("soft_skill_modal.adding") : $t("soft_skill_modal.add_new_soft_skill") }}
                            </h3>
                            <p class="text-purple-600 text-sm">
                                {{ isSaving ? $t("soft_skill_modal.please_wait") : $t("soft_skill_modal.add_soft_skill_hint") }}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div v-if="isLoading" class="flex flex-col items-center justify-center py-12">
                    ...
                    <p class="text-gray-500 text-lg">{{ $t("soft_skill_modal.loading") }}</p>
                    <p class="text-gray-400 text-sm">{{ $t("soft_skill_modal.loading_hint") }}</p>
                </div>
            </div>
            ...
            <div class="bg-gray-50 px-2 py-2 md:px-6 md:py-4 border-t border-gray-200 space-y-1">
                <div class="text-xs text-gray-500">
                    {{ $t("soft_skill_modal.total_soft_skills", { count: softSkillData.length }) }}
                </div>
                <div class="flex items-center justify-end space-x-3">
                    ...
                    >
                        ...
                        <span>{{ isSaving ? $t("soft_skill_modal.saving") : $t("soft_skill_modal.save") }}</span>
                    </button>
                </div>
            </div>
        </div>
