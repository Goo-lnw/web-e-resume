<template>
    <label class="relative inline-flex items-center cursor-pointer">
        <input type="checkbox" class="sr-only peer" :checked="locale === secondLocale" @change="toggleLocale" />
        ...
        <div class="flex justify-between items-center">
            <span class="absolute text-xs left-2 text-white transition duration-300 opacity-100" :class="{ hidden: spanRef }">TH</span>
            <span class="absolute text-xs left-7 text-white transition duration-300 opacity-100" :class="{ hidden: !spanRef }">EN</span>
        </div>
    </label>
</template>
