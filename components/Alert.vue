...
const alertTypeClass = computed(() => {
  switch (alertType.value) {
    case "success":
      return "bg-green-600 text-white shadow-green-500/50 hover:shadow-green-600/70";
    case "error":
      return "bg-red-600 text-white shadow-red-500/50 hover:shadow-red-600/70";
    case "info":
      return "bg-yellow-600 text-white shadow-yellow-500/50 hover:shadow-yellow-600/70";
    default:
      return "bg-gray-600 text-white shadow-gray-500/50 hover:shadow-gray-600/70";
  }
});
</script>
...
<template>
  <div
    v-if="isAlertVisible"
    :class="[
      'fixed top-4 right-4 sm:top-6 sm:right-6 z-50 flex items-center px-4 py-3 sm:px-6 sm:py-4 rounded-xl shadow-lg max-w-[90%] sm:max-w-sm md:max-w-md w-full font-[Mitr] animate-fade-slide-in transition-all duration-300 ease-in-out transform hover:scale-105 hover:shadow-xl',
      alertTypeClass,
    ]"
  >
    <!-- ส่วนไอคอนและข้อความ -->
    <div class="flex items-center space-x-3 flex-grow">
      ...
      <svg
        v-if="alertType === 'info'"
        class="sm:w-5 h-5 sm:h-6 animate-icon-bounce"
        xmlns="http://www.w3.org/2000/svg"
        width="24"
        height="24"
        viewBox="0 0 24 24"
    ...
    <!-- ปุ่มปิด -->
    <button
      class="ml-2 sm:ml-4 text-white hover:text-gray-200 focus:outline-none transform hover:scale-110 transition-transform duration-200 cursor-pointer"
